...
st.set_page_config(page_title="Marketing AI (HuggingFace Edition)", page_icon="🎨", layout="wide")

# --- 2. Secrete ---
try:
    GOOGLE_API_KEY = st.secrets["GOOGLE_API_KEY"]
    TAVILY_API_KEY = st.secrets["TAVILY_API_KEY"]
    HF_API_KEY = st.secrets["HF_API_KEY"] # <--- Cheia Hugging Face
except:
    st.error("⚠️ Lipsesc cheile API! Asigură-te că ai 'HF_API_KEY' în secrets.")
    st.stop()
...
# --- FUNCȚIE GENERARE IMAGINE (HUGGING FACE) ---
def generate_image_hf(prompt_text):
    """
    Folosește API-ul gratuit Hugging Face (Model SDXL).
    """
    # Endpoint pentru Stable Diffusion XL (foarte bun pt produse)
    ...
    payload = {
        "inputs": prompt_text,
        "parameters": {"num_inference_steps": 25} # Calitate standard
    }

    # Retry logic (uneori modelul 'doarme' și trebuie trezit)
    for attempt in range(4):
        try:
            ...
            # Eroare comună: "Model is loading" (așteptăm și reîncercăm)
            elif "loading" in response.text.lower():
                wait_time = response.json().get("estimated_time", 10)
                print(f"Modelul se încarcă... Aștept {wait_time} secunde.")
                time.sleep(wait_time + 1) # Așteptăm
                continue
            
            else:
                print(f"Eroare HF: {response.status_code} - {response.text}")
                time.sleep(2)
...
def create_presentation_with_images(slides_json, progress_callback=None):
    ...
    slide.placeholders[1].text = "Generat cu Gemini & Stable Diffusion"
    ...
    for i, slide_data in enumerate(data.get("slides", [])):
        ...
        if progress_callback:
            progress_callback(int((i/total)*80)+10, f"Generez Slide {i+1}/{total} (SDXL)...")
...
st.title("🎨 Asistent Marketing (Hugging Face)")
...
if prompt := st.chat_input("Ex: Vreau produse office moderne"):
...
if "last_analysis" in st.session_state:
    st.divider()
    if st.button("✨ Generează Prezentare (Cu Imagini HF)"):
        ...
        try:
            ...
            {{
                ...
                "slides": [
                    {{ "title": "Titlu", "points": ["Punct 1"], "image_prompt": "Modern office desk with blue notebook" }}
                ]
            }}
            """
            ...
            if pptx_path:
                with open(pptx_path, "rb") as f:
                    st.download_button("📥 Descarcă PPTX", f, "Prezentare_HF.pptx", "application/vnd.openxmlformats-officedocument.presentationml.presentation")
            else:
